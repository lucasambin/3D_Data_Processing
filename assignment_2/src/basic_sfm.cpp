...
struct ReprojectionError
{
  ...
  : observed_x(observed_x), observed_y(observed_y), observed_K(K)
  {}
  ...
  bool operator()(const T* const camera, const T* const point, T* residuals) const
  {
      ...
      // Remember that we are dealing with a normalized, canonical camera => Identity matrix
      T predicted_x = xp * observed_K(0, 0);
      T predicted_y = yp * observed_K(1, 1);
  ...
  static ceres::CostFunction* Create(const double observed_x, const double observed_y, const Eigen::Matrix3d& K)
  {
...
void BasicSfM::solve()
{
  ...
  while( !seed_found )
  {
    ...
    cv::Mat H = cv::findHomography(points0, points1, inlier_mask_H, cv::RANSAC, 0.001);

    // Check that the number of inliers for the model E is higher than the number of inliers for the model H
    int count_E = 0;
    int count_H = 0;
    for (int i = 0; i < inlier_mask_E.rows && i < inlier_mask_H.rows; i++)
    {
        if(inlier_mask_E.at<unsigned char>(i)) // =1
        {
            count_E++;
        }
        if(inlier_mask_H.at<unsigned char>(i)) // =1
        {
            count_H++;
        }
    }

    std::cout << "camera[" << ref_cam_pose_idx <<  "] - camera[" << new_cam_pose_idx << "]" << std::endl;
    // If the number of inliers for the model E is higher than the number of inliers for the model H
    if(count_E > count_H)
    {
        cv::recoverPose(E, points0, points1, intrinsics_matrix, init_r_mat, init_t_vec, inlier_mask_E);
        // If the magnitude of the x-component or y-component of init_t_vec is greater than the magnitude of the z-component, then the motion is predominantly sideward
        double tx = std::fabs(init_t_vec.at<double>(0));
        double ty = std::fabs(init_t_vec.at<double>(1));
        double tz = std::fabs(init_t_vec.at<double>(2));

        if(tx > tz || ty > tz)
        {
            // Predominantly sideward motion
            std::cout<<"seed_found: True" << std::endl;
            seed_found = true;
        }
        else
        {
            // Predominantly forward motion
            std::cout<<"seed_found: False" << std::endl;
            seed_found = false;
        }
    }
    // The number of inliers for the model H is higher than the number of inliers for the model E
    else
    {
        std::cout<<"seed_found: False" << std::endl;
        seed_found = false;
      ...
      if( cam_pose_optim_iter_[cam_idx] > 0 )
      {
        for( auto const& co_iter : cam_observation[cam_idx] )
        {
              ...
              cam_observation[new_cam_pose_idx].find(pt_idx ) != cam_observation[new_cam_pose_idx].end() )
          {
            ...
            // Convert the rotation parameters from axis-angle to a 3x3 rotation matrix for both cameras
            cv::Mat R0, R1;
            ...
            int new_cam_pose_obs_i = cam_observation[new_cam_pose_idx][pt_idx];
            int cam_obs_i = cam_observation[cam_idx][pt_idx];
            points0.at(0).x = observations_[2 * new_cam_pose_obs_i];
            points0.at(0).y = observations_[2 * new_cam_pose_obs_i + 1];
            points1.at(0).x = observations_[2 * cam_obs_i];
            points1.at(0).y = observations_[2 * cam_obs_i + 1];

            cv::triangulatePoints(proj_mat0, proj_mat1, points0, points1, hpoints4D);

            // Check cheirality constraint, so the depth of the point should be positive
            double depth = hpoints4D.at<double>(2)/hpoints4D.at<double>(3);
            if (depth > 0.0)
            {
                // The point isn't behind one of the cameras, store it
                n_new_pts++;
                pts_optim_iter_[pt_idx] = 1;
                double *pt = pointBlockPtr(pt_idx);
                pt[0] = hpoints4D.at<double>(0)/hpoints4D.at<double>(3); // x
                pt[1] = hpoints4D.at<double>(1)/hpoints4D.at<double>(3); // y
                pt[2] = hpoints4D.at<double>(2)/hpoints4D.at<double>(3); // z
            }
  ...
  while( keep_optimize )
  {
    ...
    for( int i_obs = 0; i_obs < num_observations_; i_obs++ )
    {
      ...
      if( cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0 )
      {
        ...
        // 2-dimensional residual. Internally, the cost function stores the observed
        // image location plus the intrinsics matrix and compares the reprojection against the observation
        double* camera = cameraBlockPtr (cam_pose_index_.at(i_obs));
        double* point = pointBlockPtr (point_index_.at(i_obs));
        static const Eigen::Matrix3d K = Eigen::Matrix3d::Ones(); // create 3x3 identity matrix
        ceres::CostFunction* cost_function = ReprojectionError::Create(observations_.at(i_obs * 2), observations_.at(i_obs * 2 + 1), K);
